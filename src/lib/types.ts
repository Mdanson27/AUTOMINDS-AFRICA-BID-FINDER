...
export type Bid = {
  ...
  firstSeenAt: string; lastSeenAt: string; sources: BidSourceRef[]; deadlineChanged?: boolean; deadlineChangedAt?: string;
};
export type SourceHealth = "healthy" | "warning" | "paused" | "planned";
export type BidSource = { id: string; name: string; type: string; baseUrl: string; health: SourceHealth; enabled: boolean; lastSuccessfulCrawlAt?: string; lastAttemptedCrawlAt?: string; recordsFound?: number; recordsCreated?: number; recordsUpdated?: number; recordsUnchanged?: number; lastError?: string };
