    ...
    <div className={`bid-list ${compact ? "compact" : ""}`}>
      {bids.map((bid) => {
        ...
        return <article className="bid-row" key={bid.id}>
          ...
          <div className="bid-actions"><button onClick={() => user && toggle(bid.id)} className={`icon-button ${saved ? "saved" : ""}`} aria-label={saved ? "Remove saved bid" : "Save bid"}>{saved ? <BookmarkCheck size={18} /> : <Bookmark size={18} />}</button><Link href={detailHref} className="icon-button" aria-label="View bid"><ExternalLink size={18} /></Link></div>
        </article>;
      })}
    </div>
  );
