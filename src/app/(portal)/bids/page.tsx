...
import { Building2, CalendarRange, Check, ChevronDown, Globe2, Landmark, Newspaper, Radar, Search, SlidersHorizontal, UsersRound } from "lucide-react";
...
export default function BidsPage() {
  ...
  const filtered = useMemo(() => {
    ...
    const result = bids.filter((bid) => {
      ...
      const matchesScope = scope === "custom"
        ? customSources.length === 0 || bid.sources.some((item) => customSources.includes(item.name))
        : sourceMatchesScope(names, scope);
  ...
  function reset() {
    setSearch(""); setStatus("all"); setCategory("all"); setSource("all"); setScope("all");
  ...
  return (
    <div className="page-stack suite-find-page">
      <PageHeader
        ...
        title="Find bids"
        description="Search current procurement intelligence by source, organization, requirement or reference. Results are limited to the active FY 2026/27 window and deadlines no more than 90 days ahead."
      ...
      <section className="panel suite-search-console">
        <div className="suite-search-question">
          <div><span className="eyebrow">SEARCH COVERAGE</span><h2>Where should we search?</h2><p>Search all detected sources or restrict results to the procurement channel that actually published the notice.</p></div>
        ...
        <label className="suite-main-search"><Search size={21} /><input value={search} onChange={(e) => setSearch(e.target.value)} placeholder="Search full title, organization, reference, service or requirement…" /><span>{filtered.length} result{filtered.length === 1 ? "" : "s"}</span></label>
